...
```{r, include=FALSE}
knitr::opts_chunk$set(engine.path = list(python = "/home/WUR/swink019/miniconda3/envs/vectorpython/bin/python"))
```
...
Before we continue with scripting, the Python environment needs to be setup. Let's use Conda. Check in Bash what environments are available.

```{r, eval=FALSE,engine='bash'}
conda info --envs
```

We won't use the existing environment, but it is a good practice to be aware of all available Conda environments. Now create a new Conda environment called vectorpython and install the required packages for today.
...
It was the [Spyder IDE](https://docs.spyder-ide.org/)! Run the bash code below to setup your file system and start Spyder. When Spyder is started in a terminal with an active conda environment, Spyder automatically uses the Python interpreter of the active conda environment.
...
Geometries in Python can be created and stored as a data type by using Shapely. Shapely can create geometries and performs geometric operations. GeoPandas uses the geometries from Shapely to store geometries in GeoSeries and GeoDataFrames. Copy the code below in the Spyder Editor and press run (F5):

```{python, engine.path="~/miniconda3/envs/vectorpython/bin/python"}
...
You just created a point in shapely and a GeoSeries in GeoPandas. A GeoSeries is a vector (a column of values) where each entry in the vector is a set of shapes corresponding to one observation. GeoPandas implements two main data structures, a GeoSeries and a GeoDataFrame. If you are familiar with Pandas, then know that these are subclasses of pandas Series and DataFrame, respectively.
...
GeoDataFrames can be loaded from a file, created with data or loaded from a Pandas dataframe. A Pandas DataFrame is the dataframe equivalent of R in Python. If a Pandas dataframe has two columns filled with coordinate pairs, then these columns can be converted to a list of Point geometries and can then be converted into a GeoSeries or directly to a GeoDataFrame.
...
A GeoDataFrame can be written directly to a GeoJSON file or a shapefile. GeoJSON is a recommended format to use since JSON dictionaries are easy to read and use on the web, and GeoJSON is supported in popular GIS software. [GeoJSON](http://geojson.org/) is a standard format to encode Geographic data structures in a dictionary. Write some files to a GeoJSON and shapefile:
...
geometry = [Point(xy) for xy in zip(df['x'], df['y'])]
wageningenGDF = gpd.GeoDataFrame(df, geometry=geometry)
wageningenGDF.to_file(filename='./wageningenPOI.geojson', driver='GeoJSON')
wageningenGDF.to_file(filename='./wageningenPOI.shp', driver='ESRI Shapefile')
```
...
```{python, engine.path="~/miniconda3/envs/vectorpython/bin/python"}
import geopandas as gpd
jsonGDF = gpd.read_file('./wageningenPOI.geojson')
shpGDF = gpd.read_file('./wageningenPOI.shp')
```
...
```{python, engine.path="~/miniconda3/envs/vectorpython/bin/python", eval=FALSE}
...
Folium uses leaflet on the backend to make webmaps. With leaflet your geodata can be easily vizualized inside a webpage. Folium works with GeoDataFrames or JSON files. The Python script below makes a html file in your working directory, which you can open in your browser: 
...
OSMnx can store the downloaded street network (the Graph) as a shapefile or as a GeoDataFrame. Furthermore, the main purpose is to perform a network analysis, such as a shortest path from source to target location.
...
Hint: If you need some help how to make functions or how to import modules, have a look at the Python refresher.
