---
pagetitle: "Tutorial 9: Vector data handling with Python"
...
output:
  knitrBootstrap::bootstrap_document:
    title: "Tutorial 9: Vector data handling with Python"
    theme: "simplex"
...
Use this file to create an environment for this tutorial, as you have learned in the previous tutorial. Next, create a second text file, (re)name it (to) `vector2.yaml`, and copy the following content into the file:
...
```{r, eval=FALSE,engine='bash'}
mamba env update --file vector2.yaml
```

Mamba automatically knows which environment to update, because of the `name` in `vector2.yaml`. Finally, activate the environment, open Spyder, create a script in the root folder and start coding.
...
There are multiple options to visualize your geodata: GIS software (QGIS), web maps (leaflet/Folium) and images (Matplotlib). We have already explored some of them previously during the tutorials, but we will go a bit more in depth in this section. Let's first take a look at creating interactive web maps using Folium.
...
Another visualization option is Matplotlib, which we have already explored during this and the previous tutorial. Matplotlib allows you to make good quality maps for reports or publications. Here is another example of its capabilities, including some of what we explored in the tutorial today.
