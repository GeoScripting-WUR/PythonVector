...
* Determine how much area (m2 and percentage) of the WUR campus are covered by buildings by using the [BAG WFS](https://geodata.nationaalgeoregister.nl/bag/wfs?) (typename = bag:pand) and visualize both the buildings surface area and the campus area in a map.
...
* Work with a project structure, make a main.py module that imports functions from another Python module called 'MyFunctions_VectorExc.py'. At least in this module we expect the following three functions:
    1. GeocodePlacenameToGDF
    2. BAGtoGeoDataFrame
    3. CalculatePercentageArea
* We expect a couple of resulting variables that are:
    1. WURbuildingsGDF => GeoDataFrame with all building geometries within the WUR campus area
    2. BuildingsArea => The total surface area (m2) of all building geometries together within the WUR campus area 
    3. PercentageCoverage => The percentage of building coverage within the total WUR campus area
...
2. Calculate how much area (m2 and %) is building surface area for the Wageningen campus.
3. Present a nice-looking map visualization of the campus and the buildings.

Optional bonus: Calculate how much area (m2 and %) is building, <u>per parcel</u>. Present result nicely summarized in one table as also in a nice-looking map visualization of the relevant parcels and their (partial) coverage by buildings. For this you will need the [Kadaster WFS](https://geodata.nationaalgeoregister.nl/kadastralekaartv3/wfs) (typename = kadastralekaartv3:perceel). 
