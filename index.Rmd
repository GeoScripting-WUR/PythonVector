...
pagetitle: "Week 3, Lesson 9: Python for vector processing | geo-scripting"
output:
...
# Week 3, Lesson 9: Vector data handling with Python
...
The conda environment we are using today contains more (and larger) packages than yesterday. Because the Virtual Machine has limited memory, it cannot create this conda environment at once. A solution is to create a conda environment with one `.yaml` file, and then update it with a second `.yaml` file.

Hereto, create a text file, (re)name it (to) `vector1.yaml`, and copy the following content into the file:
```
name: vector
dependencies:
  - python
...
Use this file to create an environment for this tutorial, as you have learned in the previous lesson. Next, create a second text file, (re)name it (to) `vector2.yaml`, and copy the following content into the file:
...
dependencies:
  ...
  - contextily
```
Now, update the environment you created before with:

```{r, eval=FALSE,engine='bash'}
conda env update --file vector2.yaml
```

Conda automatically knows which environment to update, because of the `name` in `vector2.yaml`. Finally, activate the environment, open Spyder, create a script in the root folder and start coding.
...
Geopandas provides a high-level interface to the matplotlib library (see previous lesson) for visualization. Vector data can simply be mapped by using the plot() method on a GeoSeries or GeoDataFrame. Several other arguments to customize the plot can still be used. Note that the aspect of the axes (see previous tutorial) is set to equal automatically when using Geopandas plot, i.e. the horizontal and vertical scale are automatically made the same.
...
```{Python, eval=FALSE}
# Create some data, with three points, a, b, and c, same as before
data = {'name': ['a', 'b', 'c'],
        'x': [173994.1578792833, 173974.1578792833, 173910.1578792833],
        'y': [444135.6032947102, 444186.6032947102, 444111.6032947102]}
...
# Define the crs of the data and check again
...
# print to check the contents of the WFS
...
# Check and plot to get a quick view
print(type(roadsGDF))
roadsGDF.plot()
```
...
Load some building from another WFS service (BAG) and plot them too. 
...
# Get the features for the study area
# notice that we get them as json, in contrast to before
...
Columns can be selected using the name of the column. Let us take the construction year ('bouwjaar') of the buildings.
...
Similarly, the isin() method of Series returns a boolean vector that is true wherever the Series elements exist in the passed list. This allows selecting rows where one or more columns have the required value(s). Here we select buildings that are under construction (status = building phase has started ('Bouw gestart')).
...
In summary, the advantage of GeoPandas is that it allows both geometric and dataframe manipulations. For example, GeoPandas can select the roads within (and maintained by) Wageningen Municipality and calculate total road surface area.
...
There are multiple solutions to visualize your geodata: GIS software (QGIS), web maps (leaflet/Folium) and images (matplotlib).
...
# Make choropleth map of building construction years
# Note that a continuous color scale is used as if the attribute is continuous, while it is not
...
> **Hint**: If you are using Spyder, you can view the interactive map by opening the html file that has been generated (`./output/campusMap.html`). If you are running in the Jupyter Notebook, the interactive map should automatically show.
```
...
Be aware, matplotlib supports automatic legend creation for colorbars (continuous data, see above) as well as for point and lines marker, but not for polygons. A legend with polygon shapes should be made manually using proxy artists, see [the legend guide](https://matplotlib.org/stable/tutorials/intermediate/legend_guide.html). In the example above, you would need this if you want a square in the legend with the label 'building'.
